...
def run_check(
    ...
    blocking = result.failures_at_or_above(threshold) if threshold else result.failed_findings
...
def _render_text(result: EvaluationResult, document: InfrastructureDocument) -> None:
    table = Table(title="infra-contract")
    table.add_column("Category")
    table.add_column("Result")

    for category, score in result.category_scores.items():
        failed = any(f.category == category and not f.passed for f in result.findings)
        has_critical = any(
