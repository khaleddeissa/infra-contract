...
`infra_contract.engine.evaluator.Evaluator`, the exact same providers, and
...
from infra_contract.engine.risk import classify_all, requires_human_approval
...
def infra_contract_get_policy(policy_id: str | None = None) -> str:
    ...
    """
    from infra_contract.policies.builtin import ALL_BUILTIN_POLICIES, POLICY_BY_ID
    ...
    return json.dumps(
        [p.id for p in ALL_BUILTIN_POLICIES if contract.is_policy_enabled(p.id)]
    )
...
def infra_contract_validate_change(resource_type: str, attributes_json: str) -> str:
    ...
    if result.failed_findings:
        return json.dumps({"decision": "DENIED", "violations": [f.to_dict() for f in result.failed_findings]}, indent=2)
