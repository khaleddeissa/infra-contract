...
sees `infra_contract.ir.model.InfrastructureDocument`.
...
from infra_contract.ir.model import (
    ChangeAction,
...
from infra_contract.providers.base import InfrastructureProvider
...
def _extract_db_instance(address: str, attrs: dict[str, Any]) -> Resource:
    return Resource(
        ...
        reliability=ReliabilityProperties(
            backups_enabled=(attrs.get("backup_retention_period") or 0) > 0
...
def _extract_ec2_instance(address: str, attrs: dict[str, Any]) -> Resource:
    return Resource(
        ...
        environment=_infer_environment(address, attrs),
        network=NetworkProperties(
            public=bool(attrs.get("associate_public_ip_address", False))
        ),
    )
