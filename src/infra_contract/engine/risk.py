...
from infra_contract.contracts.models import Contract
from infra_contract.ir.model import ChangeAction, Resource, ResourceKind
...
def classify_change(resource: Resource, contract: Contract) -> ChangeRisk:
    ...
    if resource.kind == ResourceKind.IAM and resource.action != ChangeAction.NO_OP:
        return ChangeRisk(
            resource.id, RiskLevel.HIGH, "IAM permissions are changing."
        )
